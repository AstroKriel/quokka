...
public:
  ...
  static constexpr int nstartHyperbolic_ =
      RadSystem<problem_t>::nstartHyperbolic_;
  ...
  int fofcMaxIterations_ =
      3; // maximum number of flux correction iterations -- only 1 is needed in
         // almost all cases, but in rare cases a second iteration is needed

  int integratorOrder_ = 2;     // 1 == forward Euler; 2 == RK2-SSP (default)
  int reconstructionOrder_ = 3; // 1 == donor cell; 2 == PLM; 3 == PPM (default)
  int radiationReconstructionOrder_ =
      3; // 1 == donor cell; 2 == PLM; 3 == PPM (default)
  ...
  explicit RadhydroSimulation(amrex::Vector<amrex::BCRec> &boundaryConditions)
    ...
    componentNames_ = {"gasDensity",    "x-GasMomentum", "y-GasMomentum",
                       "z-GasMomentum", "gasEnergy",     "scalar",
                       "radEnergy",     "x-RadFlux",     "y-RadFlux",
                       "z-RadFlux"};
  }
  ...
  void ComputeDerivedVar(int lev, std::string const &dname, amrex::MultiFab &mf,
  ...
  auto expandFluxArrays(std::array<amrex::FArrayBox, AMREX_SPACEDIM> &fluxes,
                        int nstartNew, int ncompNew)
  ...
  // radiation subcycle
  void swapRadiationState(amrex::MultiFab &stateOld,
                          amrex::MultiFab const &stateNew);
  auto computeNumberOfRadiationSubsteps(int lev, amrex::Real dt_lev_hydro)
      -> int;
  void advanceRadiationSubstepAtLevel(int lev, amrex::Real time,
                                      amrex::Real dt_radiation, int iter_count,
                                      int nsubsteps,
                                      amrex::YAFluxRegister *fr_as_crse,
                                      amrex::YAFluxRegister *fr_as_fine);
  void subcycleRadiationAtLevel(int lev, amrex::Real time,
                                amrex::Real dt_lev_hydro,
                                amrex::YAFluxRegister *fr_as_crse,
                                amrex::YAFluxRegister *fr_as_fine);

  void operatorSplitSourceTerms(
      amrex::Array4<amrex::Real> const &stateNew, const amrex::Box &indexRange,
      amrex::Real time, double dt,
      amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> const &dx,
      amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> const &prob_lo,
...
template <typename problem_t>
auto RadhydroSimulation<problem_t>::computeNumberOfRadiationSubsteps(
    int lev, amrex::Real dt_lev_hydro) -> int {
      ...
      if (is_hydro_enabled_) {
        ...
        amrex::ParallelFor(
            indexRange, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
              amrex::Real const maxSignalRadiation =
                  maxSignal(i, j, k) / static_cast<double>(maxSubsteps);
              maxSignal(i, j, k) =
                  std::max(maxSignalRadiation, maxSignalHydro(i, j, k));
            });
      }
    } else {
...
template <typename problem_t>
void RadhydroSimulation<problem_t>::checkHydroStates(amrex::MultiFab &mf,
                                                     char const *file,
                                                     int line) {
  ...
  for (amrex::MFIter iter(mf); iter.isValid(); ++iter) {
    ...
    if (!HydroSystem<problem_t>::CheckStatesValid(indexRange, state)) {
      amrex::Print() << "Hydro states invalid (" + std::string(file) + ":" +
                            std::to_string(line) + ")\n";
      amrex::Print() << "Writing checkpoint for debugging...\n";
      amrex::MFIter::allowMultipleMFIters(true);
      WriteCheckpointFile();
      amrex::Abort("Hydro states invalid (" + std::string(file) + ":" +
                   std::to_string(line) + ")");
    }
  }
}
...
void RadhydroSimulation<problem_t>::setInitialConditionsAtLevel(int lev) {
  ...
  for (amrex::MFIter iter(state_new_[lev]); iter.isValid(); ++iter) {
    ...
    setInitialConditionsOnGrid(state, indexRange, dx, prob_lo);
  }
}

template <typename problem_t>
void RadhydroSimulation<problem_t>::setInitialConditionsOnGrid(state,
                                                               indexRange, dx,
                                                               prob_lo) {
  // do nothing
}
...
template <typename problem_t>
void RadhydroSimulation<problem_t>::ComputeDerivedVar(int lev,
...
template <typename problem_t>
void RadhydroSimulation<problem_t>::computeReferenceSolution(
    amrex::MultiFab &ref,
    amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> const &dx,
    amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> const &prob_lo) {
...
template <typename problem_t>
void RadhydroSimulation<problem_t>::computeAfterEvolve(
  ...
  double const avg_rad_subcycles = static_cast<double>(radiationCellUpdates_) /
                                   static_cast<double>(cellUpdates_);
...
void RadhydroSimulation<problem_t>::advanceHydroAtLevel(
  ...
  for (amrex::MFIter iter(state_new_[lev]); iter.isValid(); ++iter) {
    ...
    if (redoFlag.max<amrex::RunOn::Device>() != quokka::redoFlag::none) {
      ...
      auto FOFluxArrays =
          computeFOHydroFluxes(stateOld, indexRange, ncompHydro_);

      for (int i = 0; i < fofcMaxIterations_; ++i) {
        if (Verbose()) {
          std::cout << "[FOFC-1] iter = " << i
                    << ", ncells = " << redoFlag.sum<amrex::RunOn::Device>(0)
                    << "\n";
        }

        // replace fluxes in fluxArrays with first-order fluxes at faces of
        // flagged cells
        replaceFluxes(fluxArrays, FOFluxArrays, redoFlag, indexRange,
                      ncompHydro_);
        ...
        HydroSystem<problem_t>::PredictStep(
            stateOld, stateNew,
            {AMREX_D_DECL(fluxArrays[0].const_array(),
                          fluxArrays[1].const_array(),
                          fluxArrays[2].const_array())},
            dt_lev, geom[lev].CellSizeArray(), indexRange, ncompHydro_,
            redoFlag.array());
  ...
  if (integratorOrder_ == 2) {
    ...
    for (amrex::MFIter iter(state_new_[lev]); iter.isValid(); ++iter) {
      ...
      amrex::FArrayBox stateFinalFAB =
          amrex::FArrayBox(indexRange, ncompHydro_, amrex::The_Async_Arena());
      ...
      if (redoFlag.max<amrex::RunOn::Device>() != quokka::redoFlag::none) {
        // compute first-order fluxes (on the whole FAB)
        auto FOFluxArrays =
            computeFOHydroFluxes(stateInter, indexRange, ncompHydro_);

        for (int i = 0; i < fofcMaxIterations_; ++i) {
          if (Verbose()) {
            std::cout << "[FOFC-2] iter = " << i
                      << ", ncells = " << redoFlag.sum<amrex::RunOn::Device>(0)
                      << "\n";
          }
          ...
          // re-do RK stage update for *all* cells
          // (since neighbors of problem cells will have modified states as
          // well)
          HydroSystem<problem_t>::AddFluxesRK2(
              stateFinal, stateOld, stateInter,
              {AMREX_D_DECL(fluxArrays[0].const_array(),
...
void RadhydroSimulation<problem_t>::replaceFluxes(
    ...
    std::array<amrex::FArrayBox, AMREX_SPACEDIM> &FOfluxes,
    amrex::IArrayBox &redoFlag, amrex::Box const &validBox, const int ncomp) {
...
auto RadhydroSimulation<problem_t>::expandFluxArrays(
    std::array<amrex::FArrayBox, AMREX_SPACEDIM> &fluxes, const int nstartNew,
  ...
  return {AMREX_D_DECL(copyFlux(fluxes[0]), copyFlux(fluxes[1]),
                       copyFlux(fluxes[2]))};
}
...
auto RadhydroSimulation<problem_t>::computeHydroFluxes(
  ...
  HydroSystem<problem_t>::ConservedToPrimitive(consVar, primVar.array(),
                                               ghostRange);
  ...
  AMREX_D_TERM(
      HydroSystem<problem_t>::template ComputeFlatteningCoefficients<
          FluxDir::X1>(primVar.array(), x1Flat.array(), flatteningRange);
      , HydroSystem<problem_t>::template ComputeFlatteningCoefficients<
            FluxDir::X2>(primVar.array(), x2Flat.array(), flatteningRange);
      , HydroSystem<problem_t>::template ComputeFlatteningCoefficients<
            FluxDir::X3>(primVar.array(), x3Flat.array(), flatteningRange);)
...
#endif
  AMREX_D_TERM(
      hydroFluxFunction<FluxDir::X1>(primVar.array(), x1Flux, x1Flat.array(),
                                     x2Flat.array(), x3Flat.array(), indexRange,
                                     nvars);
      , hydroFluxFunction<FluxDir::X2>(primVar.array(), x2Flux, x1Flat.array(),
                                       x2Flat.array(), x3Flat.array(),
                                       indexRange, nvars);
      , hydroFluxFunction<FluxDir::X3>(primVar.array(), x3Flux, x1Flat.array(),
                                       x2Flat.array(), x3Flat.array(),
                                       indexRange, nvars);)
...
void RadhydroSimulation<problem_t>::hydroFluxFunction(
  ...
  // N.B.: A one-zone layer around the cells must be fully reconstructed for
  // PPM.
  amrex::Box const &reconstructRange = amrex::grow(indexRange, 1);
  amrex::Box const &x1ReconstructRange =
      amrex::surroundingNodes(reconstructRange, dir);
  amrex::Box const &x1FluxRange = amrex::surroundingNodes(indexRange, dir);

  amrex::FArrayBox x1LeftState(x1ReconstructRange, nvars,
                               amrex::The_Async_Arena());
  amrex::FArrayBox x1RightState(x1ReconstructRange, nvars,
                                amrex::The_Async_Arena());
...
auto RadhydroSimulation<problem_t>::computeFOHydroFluxes(
  ...
  amrex::FArrayBox x3Flux(x3FluxRange, nvars,
                          amrex::The_Async_Arena()); // node-centered in z
#endif
  AMREX_D_TERM(hydroFOFluxFunction<FluxDir::X1>(primVar.array(), x1Flux,
                                                indexRange, nvars);
               , hydroFOFluxFunction<FluxDir::X2>(primVar.array(), x2Flux,
                                                  indexRange, nvars);
               , hydroFOFluxFunction<FluxDir::X3>(primVar.array(), x3Flux,
                                                  indexRange, nvars);)
...
template <typename problem_t>
void RadhydroSimulation<problem_t>::swapRadiationState(
    amrex::MultiFab &stateOld, amrex::MultiFab const &stateNew) {
  // copy radiation state variables from stateNew to stateOld
  amrex::MultiFab::Copy(stateOld, stateNew, nstartHyperbolic_,
                        nstartHyperbolic_, ncompHyperbolic_, 0);
}
...
void RadhydroSimulation<problem_t>::subcycleRadiationAtLevel(
  ...
  } else { // no hydro, or using constant dt (this is necessary for radiation
           // test problems)
  ...
  if (Verbose() != 0) {
    amrex::Print() << "\tRadiation substeps: " << nsubSteps
                   << "\tdt: " << dt_radiation << "\n";
  }
  ...
  for (int i = 0; i < nsubSteps; ++i) {
    if (i > 0) {
      // since we are starting a new substep, we need to copy radiation state
      // from 	new state vector to old state vector (this is not necessary for
...
void RadhydroSimulation<problem_t>::advanceRadiationSubstepAtLevel(
    int lev, amrex::Real time, amrex::Real dt_radiation, int const iter_count,
    int const /*nsubsteps*/, amrex::YAFluxRegister *fr_as_crse,
    amrex::YAFluxRegister *fr_as_fine) {
  // get cell sizes
  auto const &dx = geom[lev].CellSizeArray();

  // We use the RK2-SSP method here. It needs two registers: one to store the
  // old timestep, and another to store the intermediate stage (which is reused
  // for the final stage).
...
void RadhydroSimulation<problem_t>::operatorSplitSourceTerms(
  ...
  amrex::FArrayBox advectionFluxes(
      indexRange, 3,
...
auto RadhydroSimulation<problem_t>::computeRadiationFluxes(
  ...
  amrex::FArrayBox x3Flux(x3FluxRange, nvars,
                          amrex::The_Async_Arena()); // node-centered in z
  amrex::FArrayBox x3FluxDiffusive(x3FluxRange, nvars,
                                   amrex::The_Async_Arena());
#endif

  AMREX_D_TERM(fluxFunction<FluxDir::X1>(consVar, x1Flux, x1FluxDiffusive,
                                         indexRange, nvars, dx);
               , fluxFunction<FluxDir::X2>(consVar, x2Flux, x2FluxDiffusive,
                                           indexRange, nvars, dx);
               , fluxFunction<FluxDir::X3>(consVar, x3Flux, x3FluxDiffusive,
                                           indexRange, nvars, dx);)

  std::array<amrex::FArrayBox, AMREX_SPACEDIM> fluxArrays = {
      AMREX_D_DECL(std::move(x1Flux), std::move(x2Flux), std::move(x3Flux))};
  std::array<amrex::FArrayBox, AMREX_SPACEDIM> fluxDiffusiveArrays{
      AMREX_D_DECL(std::move(x1FluxDiffusive), std::move(x2FluxDiffusive),
                   std::move(x3FluxDiffusive))};
...
template <typename problem_t>
template <FluxDir DIR>
void RadhydroSimulation<problem_t>::fluxFunction(
    amrex::Array4<const amrex::Real> const &consState, amrex::FArrayBox &x1Flux,
    amrex::FArrayBox &x1FluxDiffusive, const amrex::Box &indexRange,
    const int nvars, amrex::GpuArray<amrex::Real, AMREX_SPACEDIM> dx) {
  int dir = 0;
  if constexpr (DIR == FluxDir::X1) {
    dir = 0;
  } else if constexpr (DIR == FluxDir::X2) {
  ...
  amrex::Box const &ghostRange = amrex::grow(indexRange, nghost_);
  // N.B.: A one-zone layer around the cells must be fully reconstructed in
  // order for PPM to work.
  amrex::Box const &reconstructRange = amrex::grow(indexRange, 1);
  amrex::Box const &x1ReconstructRange =
      amrex::surroundingNodes(reconstructRange, dir);
  ...
  // cell-centered kernel
  RadSystem<problem_t>::ConservedToPrimitive(consState, primVar.array(),
                                             ghostRange);
  ...
  if (radiationReconstructionOrder_ == 3) {
    ...
    amrex::Abort(
        "Invalid reconstruction order for radiation variables! Aborting...");
  }

  // interface-centered kernel
  amrex::Box const &x1FluxRange = amrex::surroundingNodes(indexRange, dir);
  RadSystem<problem_t>::template ComputeFluxes<DIR>(
      x1Flux.array(), x1FluxDiffusive.array(), x1LeftState.array(),
      x1RightState.array(), x1FluxRange, consState,
